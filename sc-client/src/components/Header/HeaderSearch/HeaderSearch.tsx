...
type HistoryState = {
  history: any[] // Replace 'any[]' with the actual type of history data
}
      ...
      <div className={`headerSearch-container ${isSearchOpen ? ' open' : ''}`}>
        <search className="headerSearch__search">
          ...
          <div className="_search_">
            <div className="_search_input">
              ...
              />
              <div>
                <button className="_search_input_borrar" onClick={handleDeleteClick}>
                  {'x '}
                  <i>borrar</i>
                </button>
                <button>
                  <FiSearch className="_search_input_fi-search" />
                </button>
              </div>
            </div>
          </div>
        </search>
        {
          <section className={`headerSearch__section ${isElementVisible || isResultsVisible ? ' open' : ''}`}>
            {isElementVisible ? (
              <SuggestionsSection props={{ localHistory, handleSuggestionClick, clearLocalHistory }} />
            ) : isResultsVisible ? (
              <ResultsSection products={productsFetched} />
            ) : null}
          </section>
        }
        {/* <section className={`headerSearch__section ${isElementVisible || isResultsVisible ? ' open' : ''}`}>
